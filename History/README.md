...
**JDK8 :**<br>
람다표현식
- 메소드를 하나의 식으로 표현한 것을 말한다. 코드의 가독성이 좋아진다.
- @FunctionalInterface로 함수형 인터페이스임을 명시할 수 있다.
- 람다 표현식을 사용하여 자바에서도 함수형 프로그래밍을 할 수 있게 되었다.
<br>
StreamAPI
- 배열이나 컬렉션에 접근할 때 반복자를 사용하는데 코드의 길이가 길어지고 가독성이 떨어지게 된다.
- Stream을 사용하면 다양한 방식으로 저장된 데이터를 읽고 쓰기 위해 공통적인 방법을 제공한다.
- parallelStream()으로 병렬처리가 가능하다.
- 원본데이터를 변경하지 않는다.
<br>
java.time 패키지
- 기존에는 Date 클래스와 Calendar 클래스를 통해서 날짜와 시간을 얻었지만 불변객체가 아니라 값이 수정될 수도 있었고 월을 0 - 11까지 표현해야 하는 불편함이 있었다.
- Joda-time 라이브러리를 발전시킨 패키지이다.

**@어노테이션 :**<br>
용도는 다양한 목적이 있지만 메타데이터의 비중이 가장 크다. <br>
메타데이터란 한 데이터에 대한 설명을 의미하는 데이터이다. <br>
Interface 앞에 "@"를 붙여 어노테이션을 만들 수 있다. <br>
원하는 어노테이션을 설정하여 리플렉션으로 원하는 목적을 수행시킬 수 있다. <br>

**.jar .war :**<br>
.jar는 하나의 application 기능이 가능하도록 java 파일 등을 압축하고 지원한다. <br>
.war는 웹 어플리케이션을 지원하기 위한 압축 방식이다. .jsp .servlet .gif .html .jar 등을 압축하고 지원한다. <br>

**의존성 주입(Dependency Injection) :**<br>
모듈 간의 의존성을 줄이고 소스 수정없이 런타임에 더욱 유연한 소프트웨어를 만들 수 있는 개발 패턴이다. <br>
특정 객체가 있을 때 특정 역할을 외부에서 생성자나 세터를 통해 주입받아서 해결하는 방식이다. <br>
<br>
IoC(Inversion of Control) 제어의 역전 :
- 의존관계를 스프링설정 파일에 등록된 < bean >을 바탕으로 컨테이너가 자동으로 처리해준다.
<br>
의존한다라는 의미는 A가 변하면 B에 영향을 미친다라는 의미이다. <br>
주로 Setter를 사용해 주입하는데 Setter 사용을 못할 시에 생성자로 주입한다. <br>
Interface로 구현이 가능하다. <br>
xml에서는 세터는 < property >를 사용하고 생성자는 < constructor-arg >를 사용하여 설정한다. <br>
xml설정 java설정이 있다. <br>
네임스페이스를 이용하여 사용할 수도 있다. <br>

**Serializable(객체직렬화) :**<br>
객체의 내용을 바이트 단위로 변환하여 파일 또는 네트워크를 통해서 스트림이 가능하게 하는 것을 의미한다. <br>
Serializable 인터페이스를 상속받은 객체는 직렬화할 수 있는 기본 조건을 가진다. <br>
자바 직렬화 대상 객체는 동일한 serialVersionUID를 가지고 있어야 한다. <br>
자바 직렬화 형태의 데이터 교환은 자바시스템간의 데이터 교환을 위해 존재한다. 하지만 매우 엄격하므로 조심해서 사용해야 한다. <br>
Jackson, GSON 라이브러리로 JSON형태로 직렬화, 역직렬화하여 사용하는 방법도 있다. <br>

**Beans :**<br>
Spring에서는 POJO를 Beans라고 한다. <br>
Beans는 어플리케이션의 핵심을 이루는 객체이면서 Spring IoC 컨테이너에 의해 인스턴스화, 생성, 관리된다. <br>
설정매타데이터(xml)에 의해 생성된다. <br>

**Reflection :**<br>
객체지향프로그래밍 언어에서는 reflection을 사용하면 실행 중에 클래스, 인터페이스, 필드 및 메서드에 접근할 수 있고 새로운 객체의 인스턴스화 및 메서드를 호출 가능케한다. <br>

**Spring context :**<br>
기본적으로 'root-context'와 'servlet-context'가 생성되는데 'root-context'에 등록된 빈들은 모든 context에서 사용가능하다. <br>
다른 context에서 등록된 빈은 해당 context에서만 유효하고 'root-context'와 Bean이 겹칠 경우 오버라이딩이 된다. <br>
'servlet-context'에는 controller를 등록하고, 'root-context'는 service, repository를 등록한다. <br>

**Apache :**<br>
요청 하나 당 프로세스 또는 스레드가 처리하는 구조로 구성된다. <br>
요청이 많을수록 CPU와 메모리 사용이 증가하기 때문에 성능 저하가 발생한다. <br>
이와 같은 현상을 keep-alive로 해결이 가능하지만 대량 접속 시에 효율이 급격히 떨어진다. <br>
요청을 유지하기 때문에 maxClient 값을 초과하게 된다. <br>
Apache 2.4 버전부터는 이를 해결할 Event MPM 방식을 지원한다. <br>
MPM은 요청을 처리하는 스레드를 따로 둬서 분산된 처리를 하게 한다. <br>

**NginX :**<br>
보안과 속도를 최적화시키는 노력으로 탄생한 웹 서버이기 때문에 사용이 심플하고 규모가 작은 서비스이면서 정적데이터 처리가 많은 서비스에 적합하다. <br>
Event Driven 방식의 웹 서비스인데 요청에 대한 상태를 정해서 event가 발생할 때마다 event를 처리하는 방식이다. <br>

**DBCP :**<br>
Database Connection Pool을 말한다. <br>
DB와 연결하는 커넥션을 미리 생성해두고 Pool에 저장해두었다가 필요할 때 꺼내쓰고 다시 풀에 반환하는 기법이다. <br>
미리 생성해두기 때문에 커넥션은 사용자가 DB를 사용할 때마다 매번 생성하는 것보다 더 빠른 속도를 보장한다. <br>
최대 생성 갯수도 제어해주기 때문에 많은 사용자가 몰려도 과부화를 방지할 수 있다. <br>

**BDD :**<br>
Behaviour Driven Development를 말한다. <br>
시나리오 기반의 테스트 케이스를 작성하며 함수단위 테스트를 권장하지 않는다. <br>
Given : 시나리오 진행에 필요한 값을 설정 <br>
When : 시나리오 진행하는데 필요한 조건을 명시 <br>
Then : 시나리오를 완료했을 때 보장해야하는 결과 <br>

**AOP :**<br>
Aspect Oriented Programming을 말한다. <br>
기능을 핵심 비즈니스 기능과 공통 기능으로 구분하고 개발자의 코드 밖에서 필요한 시점에 적용하는 프로그래밍 기법이다. <br>
Proxy객체를 이용하여 해당 기법을 구현한다. <br>

**CORS :**<br>
Cross-origin resource sharing을 말한다. <br>
서로 다른 도메인에서 리소스를 구현하는 방식이라 볼 수 있다. <br>
반대되는 언어로 Some-origin policy가 있는데 특정 도메인만 해당 API를 허가하고 싶을 때 적용한다. <br>
하지만 특정 도메인에서 사용하는 도메인이 여러 개의 도메인을 가지고 있을 경우 문제가 발생한다. <br>
그래서 CORS 설정을 해주는 방식을 사용한다. <br>

**REST :**<br>
Representational State Transfer을 말한다. <br>
자원을 이름으로 구분하여 해당 자원의 상태를 주고 받는 모든 것을 말한다. <br>
웹의 기존 기술과 프로토콜을 그대로 활용하기 때문에 웹의 장점을 최대한 활용할 수 있는 아키텍처 스타일이다. <br>
HTTP URI를 통해 자원을 명시하고 HTTP METHOD를 통해 해당 자원에 대한 CRUD를 적용한다. <br>
애플리케이션을 분리 또는 통합할 수 있는 이점이 있고, 다양한 브라우저 모바일 디바이스에서도 통신이 되어야하기 때문에 사용한다. <br>

**REST API :**<br>
REST기반으로 서비스 API를 구현한 것을 말한다. <br>
최근 OpenAPI, 마이크로 서비스 등을 제공하는 업체 대부분은 REST API를 제공한다. <br>
REST 원리를 따르는 시스템은 RESTful이란 용어로 지칭한다. <br>

**Transaction :**<br>
작업수행의 논리적 단위를 말한다. <br>
작업 수행 중 실행이 중단될 경우 rollback하고 실행을 모두 마치면 commit한다. <br>
DBMS의 성능은 초당 트랜잭션 실행 수를 의미한다. <br>

**ACID :**<br>
A : Auomicity 원자성을 말하고, 원자성을 보장하기 위해 콜백세그먼트를 이용한다. <br>
C : Consistency 일관성을 말하고, 일관성을 보장하기 위해 트리거를 이용한다. <br>
I : Isolation 격리성을 말하고, 격리성을 보장하기 위해 2PL프로토콜을 이용한다. <br>
D : Durability 격리성을 말한다. <br>

**Serverless :**<br>
BaaS(Back as a Service), FaaS(Function as a Service)에 의존하여 작업을 처리하는 방식을 말한다. <br>
BaaS란 필요한 모든 기능들을 API 호출로 사용하고 사용한 만큼의 금액을 내는 대신 서버를 신경쓰지 않아도 된다는 장점이 있다. <br>
FaaS란 함수 단위로 쪼개 배포하며 계속 실행되는 것이 아니라 특정 이벤트가 발생했을 때 실행된다. <br>

**마이크로서비스 :**<br>
먼저 모노리틱 아키텍처를 알아야하는데 모든 로직들이 하나의 애플리케이션에 들어있는 구조이다. <br>
마이크로서비스는 데이터에서부터 비즈니스로직까지 독립적으로 상호 컴포넌트간의 의존성이 없어 개발된 컴포넌트로 REST API와 같은 표준 인터페이스로 그 기능을 외부로 제공한다. <br>

**Java의 Heap, Stack :**<br>
Stack영역에는 매개변수와 지역변수가 저장된다. <br>
Heap영역에는 new 연산자를 통해 생성된 인스턴스 변수가 놓인다. <br>
Heap영역에 놓은 변수가 주소를 잃어버려 가비지가 되면 가비지 컬렉션에 의해 제거된다. <br>
힙 영역에 진짜 값이 들어있고 스택에는 그 주소만 저장되어 있다. <br>

**MVC1, MVC2, Spring MVC :**<br>
MVC1은 JSP에서 사용자의 요청을 받아 모든 정보를 표현하고 저장하고 처리하는 구조를 말한다. <br>
가독성이 떨어지고 유지보수를 어렵게 만든다. <br>
MVC2는 웹 브라우저의 요청을 하나의 서블릿이 받아 알맞게 처리한 후 그 결과를 JSP페이지로 포워딩한다. <br>
Spring MVC는 트랜잭션 처리, AOP, DI를 손쉽게 사용가능하도록 지원한다. <br>

**검색엔진 작동원리 :**<br>
링크와 텍스트 기반 인식 기본이다. 크롤러가 방문한 웹 페이지에 새로운 링크를 발견하면 웹 서버에 데이터 요청을 보내고 이때 웹 서버는 검색엔진에게 웹 페이지 정보를 보내는데 이것이 바로 메타태그가 포함된 사이트 정보이고 크롤러는 추가로 수집한 URL에서 단어와 문구를 분리하여 저장하는데 각 단어와 문구에 가중치와 연관도를 부여하고 최종 값을 인덱스한다. <br>
검색자가 검색창에 검색어를 입력하면 검색엔진이 미리 구축해두었던 인덱스를 검색하게 하는 것이다. <br>

**대용량데이터 처리 :**<br>
대규모데이터 처리가 어려운 점은 메모리 내에서 계산을 할 수 없다는 것인데 그렇기 때문에 디스크에 있는 데이터를 검색하여 처리하기 때문에 I/O 시간이 매우 많이 걸린다. <br>
방법론으로는 샤딩이나 하둡을 사용하는 방법이 있다. <br>

**Java의 GC :**<br>
자바에서는 메모리를 GC라는 알고리즘을 통해 관리한다. <br>
GC의 역할은 메모리 할당, 사용중인 메모리 인식, 사용하지 않은 메모리 인식이 있다. <br>
GC는 Heap메모리를 다룬다. <br>
Eden, Survivor1, Survivor2, Old 영역이 있는데 메모리에 객체가 생성되면 Eden에 객체가 저장되고 꽉 차게 될 시에 Survivor1 혹은 Survivor2에 옮겨지거나 삭제된다. <br>
Survivor1, Survivor2 둘 중 하나는 비워져있어야 한다. <br>
더 이상 Young영역에 자리가 없을 경우 Old영역에 저장된다. <br>
Young은 마이너 GC, Old영역은 메이저 GC에 의해 메모리가 관리된다. <br>

**hashCode() :**<br>
equals는 두 객체의 내용이 같은 지 비교하는 연산자이고 hashCode는 두 객체가 같은 객체인지 비교하는 연산자이다. <br>
equals만 재정의하면 안되고 hashCode를 함께 재정의해야 한다. <br>
euqals로 같은 값인 경우 hashCode도 값은 값이지만 그 반대의 경우는 같은 값일 수도 아닐 수도 있다. <br>

**기본키/인덱스키 :**<br>
기본키 Primary Key는 unique + not null 제약조건이 기본으로 따라온다. <br>
기본키의 목적은 데이터의 무결성이고 인덱스키는 여러 개 생성 가능하면서 빠른 조회에 목적을 두고 있다. <br>

**TDD :**<br>
Test Driven Development를 말한다. <br>
테스트 주도 개발 : 테스트가 개발을 이끌어나간다. <br>
테스트를 먼저 만들고 테스트를 통과하기 위해 코드를 짜는 것으로 제대로 동작하는지에 대해 피드백을 적극적으로 받는 것을 말한다. <br>
TDD의 목적은 피드백과 협력을 증진시키는 데에 있고 불확실성에 대비를 하게 해준다. <br>

**Http Https :**<br>
Http는 웹브라우저와 서버 간의 자원을 주고 받을 때 쓰는 통신규약인데 http는 텍스트 교환이라 누군가 가로채어 노출될 우려가 있다. <br>
때문에 이런 보안적인 이슈를 해결하기 위해서 Https가 고안된 것이다. <br>
인터넷 상에서 정보를 암호화해주는 SSL프로토콜을 이용하여 웹 브라우저와 서버가 데이터를 주고 받는 통신규약이 HTTPS이다. <br>

**공개키암호화 :**<br>
서로 다른 키 공개키, 개인키가 존재하는 데 1번키로 암호화하면 2번키로 해독할 수 있고 2번키로 암호화하면 1번키로 해독할 수 있다. <br>
공개키는 공개키 저장소에 등록해놓는데 보안상의 의미를 생각해봤을 때 내가 보낸 요청이 해당 서버로부터 온 응답임을 알 수 있다. <br>
공개키 저장소는 CA이고 기업이 CA에 계약하고 돈을 지불하면 공개키와 개인키를 받는다. <br>

**JVM 구조 :**<br>
일단 .java 파일을 .class 파일로 컴파일러에 의해 생성되고 그 후 JVM의 ClassLoader가 런타임 시에 실행될 클래스 파일들을 메모리에 로드 후 초기화 작업을 실시한다. <br>
Execution Engine은 클래스로더를 통해 JVM 내로 배치된 바이트 코드를 실행시킨다. <br>
Runtime Data Areas는 JVM이 운영체제 위에서 실행되면서 할당받은 메모리영역이다. <br>
클래스 로더에서 준비된 데이터들을 보관하는 저장소이다. <br>

**쿠키와 세션 :**<br>
사용하는 이유는 HTTP 프로토콜의 단점을 보완하기 위해서이다. <br>
HTTP는
- 비연결지향(!HTTP 1.1)이고 Stateless <br>
쿠키는 클라이언트 로컬에 저장되고 키와 값이 들어있는 작은 데이터 파일이다.(자동로그인, 장바구니) <br>
하나의 도메인 당 20개의 값만 가질 수 있다. <br>
세션은 일정시간동안 브라우저로부터 들어오는 일련의 요구를 하나의 상태로 보고 그 상태를 유지하는 기술이다. <br>
둘의 차이로는 저장장소(클라이언트, 서버), 보안(위치의 특성), 라이프사이클(브라우저 종료시 소멸되는 세션) 등이 있겠다. <br>

**자바의 특징 :**<br>
이식성이 높은 언어이다. 어떤 운영체제이던 JRE가 설치되어 있으면 코드 수정없이 작동시킬 수 있다. <br>
객체지향 언어이다. 부품에 해당하는 객체들을 만들어 이것들을 조립하고 연결해서 전체 프로그램을 완성하는 기법을 사용한다. <br>
메모리를 자동으로 관리해준다. GC에 의해 자동적으로 메모리가 관리된다. <br>
오픈소스라이브러리가 풍부하며 예외처리는 불편한 특징이 있다. <br>

**블로킹 넌블로킹 :**<br>
블로킹은 네트워크 통신에서 요청이 발생하고 완료 될 때까지 모든 일을 중단한 상태로 대기해야하는 것을 말한다. <br>
논블로킹은 네트워크 통신이 완료될 때까지 기다리지 않고 다른 작업을 수행할 수 있으므로 경우에 따라서는 효율이나 반응속도가 뛰어나나 설계가 복잡해진다. <br>
논블로킹은 비동기보다는 polling에 가깝다. <br>

**TCP / UDP :**<br>
TCP(Transmission Control Protocol) : 연결형 서비스이며 가상회선방식으로 통신한다. 전송 순서를 보장하며 신뢰성이 높고 수신여부를 확인할 수 있다. 반면에 느리다. <br>
UDP(User Datagram Protocol) : 비연결형 서비스이며 데이터그램 방식(65535)으로 전송하며 신뢰성이 낮지만 빠르다. <br>

**컴파일 :**<br>
컴파일이란 사람이 이해하기 쉬운 고급언어로 개발한 프로그램을 컴퓨터가 이해하기 쉬운 언어인 기계어로 바꿔 주는 것을 말한다. <br>
JVM excution Engine에 의해 기계어로 해석되어 메모리 상에 배치되게 된다. <br>
실행엔진에는 인터프리터와 JIT(Just in Time) 컴파일러가 있는데 인터프리터는 한 줄 씩 읽기 때문에 느리고 JIT는 적절한 시점에 바이트 코드 전체를 컴파일하고 해당 코드를 직접 실행한다. <br>

**SOLID 원칙 :**<br>
SRP(Single Responsibility Principle) : 단일 책임 원칙
- 작성된 클래스는 하나의 기능만을 가지며 클래스가 제공하는 모든 서비스는 그 하나의 책임을 수행하는데 집중되어 있어야 한다.
- 클래스를 변경해야 하는 이유는 오직 하나뿐이어야 함을 말한다.

OCP(Open Close Principle) : 개방폐쇄의 원칙
- 소프트웨어 구성요소는 확장에는 열려있고 변경에는 닫혀 있어야한다는 말이다.
- 요구사항의 변경이나 추가사항이 발생하더라도 기존 구성요소는 수정이 일어나지 말아야하며, 기존 구성요소를 쉽게 확장해서 재사용할 수 있어야 한다.
...
- 추상을 매개로 메세지를 주고 받음으로써 관계를 최대한 느슨하게 만드는 원칙이다.
